...
1. 開啟學生版首頁，可直接點左側分類，或點「最低修課學分」等範例問題。
2. 也可在下方輸入框輸入完整問題，再按「送出」。
3. 有符合 FAQ 時，系統會顯示 AI 整理後的答案、資料來源與相似度。
...
1. 開啟 `/office`。
2. 輸入安裝時產生的本處室專屬登入碼，再按「登入」。
3. 登入後只會看到分派給本處室的需求單，可依狀態篩選。
...
1. 開啟 `/admin`，輸入 `admin-token.txt` 內的管理密碼。
2. 使用上方期間選單切換統計區間，檢視全校需求單數量、轉單率、結案數與平均處理時間。
3. 在需求單清單中依處室或狀態篩選，必要時可調整承辦處室、狀態、回答及承辦人。
...
1. 開啟 `/design`，輸入管理員密碼或 `theme-admin-token.txt` 內的外觀後台密碼。
2. 調整標題、歡迎文字、分類名稱、配色、字體、字級、圓角與按鈕樣式。
3. 使用即時預覽確認學生端效果。
...
1. 使用管理員密碼登入 `/faq-admin`。
2. 先用搜尋、分類或主責單位篩選，確認要修改的 FAQ。
3. 單筆異動可按「編輯」；新增問題可按「＋ 新增一筆」。
4. 大量異動可選 Google 試算表、上傳 CSV／Excel，或直接貼上試算表內容。
5. 先按預覽，逐列檢查新增、覆蓋及略過結果。
6. 選擇「合併更新」或「完全取代」後才正式匯入。
7. 儲存後回學生端實際搜尋一次；若向量結果異常，再按「重建向量」。
